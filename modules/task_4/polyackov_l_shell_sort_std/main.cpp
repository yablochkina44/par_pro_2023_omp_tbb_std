...
TEST(Parallel_Operations_Std_Threads, Test_Sorted_Array) {
  std::vector<int> tmp = get_random_vec(num_of_elems, random_gen_seed);
  std::sort(tmp.begin(), tmp.end());
  std::vector<int> res = shell_sort_with_simple_merge_std(tmp, num_of_parts);
  for (int i = 1; i < res.size(); i++) {
    ASSERT_LE(res[i - 1], res[i]);
  }
}

TEST(Parallel_Operations_Std_Threads, Test_Reverse_Sorted_Array) {
  std::vector<int> tmp = get_random_vec(num_of_elems, random_gen_seed);
...
TEST(Parallel_Operations_Std_Threads, Test_Partially_Sorted_Array) {
  std::vector<int> tmp = get_random_vec(num_of_elems, random_gen_seed);
...
TEST(Parallel_Operations_Std_Threads, Test_Array_With_Repeating_Elements) {
  std::vector<int> tmp = get_random_vec(num_of_elems, random_gen_seed);
...
TEST(Parallel_Operations_Std_Threads, Test_Array_Whose_Values_Are_All_The_Same) {
...
int main(int argc, char **argv) {
  ...
  std::vector<int> res1 = shell_sort_with_simple_merge_std(tmp1, num_of_parts);
  auto stop1 = std::chrono::high_resolution_clock::now();
  auto duration1 = std::chrono::duration_cast<std::chrono::microseconds>(stop1 - start1);

  auto start2 = std::chrono::high_resolution_clock::now();
  std::vector<int> res2 = shell_sort_with_simple_merge_std(tmp2, num_of_parts);
