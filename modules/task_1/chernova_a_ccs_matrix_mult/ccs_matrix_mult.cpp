...
#include "../../../modules/task_1/chernova_a_ccs_matrix_mult/ccs_matrix_mult.h"
...
MatrixCSS::MatrixCSS(const std::vector<std::complex<double>>& v,
                     size_t numberRows, size_t numberColumns)
    : numberRows(numberRows), numberColumns(numberColumns) {
  values.reserve((numberRows * numberColumns) / 2);
  rows.reserve((numberRows * numberColumns) / 2);
  pointer.reserve(numberColumns + 1);
